...
export function MobileTabBar({ openSession }: { openSession: OpenSession | null }) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex items-center border-t border-border bg-bg-deep/95 backdrop-blur-md">
      ...
      <button
        onClick={() => navigate(openSession ? ROUTES.home : ROUTES.sessionStart)}
        className="flex flex-1 flex-col items-center gap-0.5 py-2.5 text-[10px] font-semibold text-primary"
      >
        <Zap size={20} />
